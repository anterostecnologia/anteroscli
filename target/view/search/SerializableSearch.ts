...
import {SerializableController} from "../controller/SerializableController";
...
import { serializable, homeDefault } from "../route/routes";

export const SERIALIZABLE_SEARCH_MODAL = "SerializableSearch";

@boundClass
class SerializableSearch extends AnterosSearchView<SerializableEntity, typeof SerializableEntity.prototype.id> {
    onCloseView() {
...
interface SerializableTableProps<E extends AnterosEntity, TypeID> {
    user: UserData;
    needRefresh: boolean;
...
@boundClass
class SerializableTable extends Component<SerializableTableProps<SerializableEntity, any>> {
    ...
    render(): ReactNode {
        ...
        return ( 
          <AnterosSearchTemplate 
            defaultSortFields="nameSerializable"  
            filterName="filterSerializableSearch" 
            version="v1" 
            caption={"Consulta Serializable"} 
            dataSource={dataSource} 
            viewName={"serializableSearch"} 
            user={user} 
            pageSize={PAGE_SIZE} 
            currentFilter={currentFilter} 
            needRefresh={needRefresh} 
...
export default connectSearchViewWithStore(resolve<SerializableController>(TYPE.serializable_controller)())(SerializableSearch);
