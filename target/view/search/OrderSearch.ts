...
export const ORDER_SEARCH_MODAL = "OrderSearch";

@boundClass
class OrderSearch extends AnterosSearchView<OrderEntity, typeof OrderEntity.prototype.id> {
    onCloseView() {
    ...
    render(): ReactNode {
        ...
        return ( 
          <AnterosSearchTemplate 
            defaultSortFields="nameOrder"  
            filterName="filterOrderSearch" 
            version="v1" 
            caption={"Consulta Order"} 
            dataSource={dataSource} 
            viewName={"orderSearch"} 
            user={user} 
            pageSize={PAGE_SIZE} 
            currentFilter={currentFilter} 
            needRefresh={needRefresh} 
...
export default connectSearchViewWithStore(resolve<OrderController>(TYPE.order_controller)())(OrderSearch);
