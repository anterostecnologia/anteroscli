...
export const TRIP_SEARCH_MODAL = "TripSearch";

@boundClass
class TripSearch extends AnterosSearchView<TripEntity, typeof TripEntity.prototype.id> {
    onCloseView() {
    ...
    render(): ReactNode {
        ...
        return ( 
          <AnterosSearchTemplate 
            defaultSortFields="nameTrip"  
            filterName="filterTripSearch" 
            version="v1" 
            caption={"Consulta Trip"} 
            dataSource={dataSource} 
            viewName={"tripSearch"} 
            user={user} 
            pageSize={PAGE_SIZE} 
...
export default connectSearchViewWithStore(resolve<TripController>(TYPE.trip_controller)())(TripSearch);
