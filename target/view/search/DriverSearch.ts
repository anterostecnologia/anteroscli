...
import { driver, homeDefault } from "../route/routes";

export const DRIVER_SEARCH_MODAL = "DriverSearch";

@boundClass
class DriverSearch extends AnterosSearchView<DriverEntity, typeof DriverEntity.prototype.id> {
    onCloseView() {
...
interface DriverTableProps<E extends AnterosEntity, TypeID> {
    user: UserData;
    needRefresh: boolean;
...
@boundClass
class DriverTable extends Component<DriverTableProps<DriverEntity, any>> {
    ...
    render(): ReactNode {
        ...
        return ( 
          <AnterosSearchTemplate 
            defaultSortFields="nameDriver"  
            filterName="filterDriverSearch" 
            version="v1" 
            caption={"Consulta Driver"} 
            dataSource={dataSource} 
            viewName={"driverSearch"} 
            user={user} 
            pageSize={PAGE_SIZE} 
            currentFilter={currentFilter} 
            needRefresh={needRefresh} 
...
export default connectSearchViewWithStore(resolve<DriverController>(TYPE.driver_controller)())(DriverSearch);
