...
interface VehicleModelViewProps extends AnterosViewProps<VehicleModelEntity, typeof VehicleModelEntity.prototype.id> {
}

interface VehicleModelViewState extends AnterosViewState {
}

@boundClass
class VehicleModelView extends AnterosView<VehicleModelEntity, typeof VehicleModelEntity.prototype.id,  VehicleModelViewProps, VehicleModelViewState> {
    onCloseView() {
...
class VehicleModelTable extends Component<VehicleModelTableProps<VehicleModelEntity, any>> {
    ...
    getRoutes(): any {
        return { 
          add: `${vehicleModel}/${ADD}`, 
          edit: `${vehicleModel}/${EDIT}`, 
          close: `${homeDefault}`, 
        };
    }
    ...
    render(): ReactNode {
        ...
        return ( 
          <AnterosTableTemplate 
            ...
            filterName="filterVehicleModel" 
            version="v1" 
            caption={"VehicleModel"} 
            dataSource={dataSource} 
            viewName={"vehicleModelView"} 
            user={user} 
            pageSize={PAGE_SIZE} 
            currentFilter={currentFilter} 
            needRefresh={needRefresh} 
...
interface VehicleModelFormProps extends AnterosFormComponentProps {
    dataSource: AnterosDatasource;
    saveRoute: string;
    cancelRoute: string;
    needRefresh: boolean;
...
interface VehicleModelFormsState extends AnterosFormComponentState {
}

@boundClass
class VehicleModelForm extends AnterosFormComponent<VehicleModelFormProps, VehicleModelFormsState> {
    constructor(props: VehicleModelFormProps) {
        super(props); 
         this.state = { 
          modalOpen: undefined, 
          lookup: "", 
...
export default connectViewWithStore(resolve<VehicleModelController>(TYPE.vehicleModel_controller)())(VehicleModelView);
